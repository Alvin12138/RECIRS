...
static inline std::string &ltrim(std::string &s) {
    #if CFI_PROTECT
    /* CFI: default, tag=0x04F5DCF4U */
    CFI_PROLOGUE_TAG(0x04F5DCF4U);
    #endif

    s.erase(s.begin(), std::find_if(s.begin(), s.end(),
            std::not1(std::ptr_fun<int, int>(std::isspace))));
    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0x04F5DCF4U);
    #endif
    return s;
}
...
static inline std::string &rtrim(std::string &s) {
    #if CFI_PROTECT
    /* CFI: default, tag=0xCFDB52EFU */
    CFI_PROLOGUE_TAG(0xCFDB52EFU);
    #endif

    s.erase(std::find_if(s.rbegin(), s.rend(),
            std::not1(std::ptr_fun<int, int>(std::isspace))).base(), s.end());
    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0xCFDB52EFU);
    #endif
    return s;
}
...
static inline std::string &trim(std::string &s) {
    #if CFI_PROTECT
    /* CFI: default, tag=0x6A8D6095U */
    CFI_PROLOGUE_TAG(0x6A8D6095U);
    #endif

    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0x6A8D6095U);
    #endif
...
public:
    ...
    virtual CommandStatus Select(const ControlRelayOutputBlock& command, uint16_t index) {
        #if CFI_PROTECT
        /* CFI: default, tag=0x70DA9295U */
        CFI_PROLOGUE_TAG(0x70DA9295U);
        #endif

        index = index + offset_di;
        #if CFI_PROTECT
        CFI_EPILOGUE_TAG_PAC(0x70DA9295U);
        #endif
        return CommandStatus::SUCCESS;
    }
    virtual CommandStatus Operate(const ControlRelayOutputBlock& command, uint16_t index, OperateType opType) {
        #if CFI_PROTECT
        /* CFI: default, tag=0xA4F6D0BEU */
        CFI_PROLOGUE_TAG(0xA4F6D0BEU);
        #endif
        ...
        #if CFI_PROTECT
        CFI_EPILOGUE_TAG_PAC(0xA4F6D0BEU);
        #endif
        return return_val;
    }
    ...
    virtual CommandStatus Select(const AnalogOutputInt16& command, uint16_t index) {
        #if CFI_PROTECT
        /* CFI: default, tag=0xB086BBADU */
        CFI_PROLOGUE_TAG(0xB086BBADU);
        #endif

        index = index + offset_ao;
        #if CFI_PROTECT
        CFI_EPILOGUE_TAG_PAC(0xB086BBADU);
        #endif
        return CommandStatus::SUCCESS;
    }
    virtual CommandStatus Operate(const AnalogOutputInt16& command, uint16_t index, OperateType opType) {
        #if CFI_PROTECT
        /* CFI: default, tag=0x19E6546AU */
        CFI_PROLOGUE_TAG(0x19E6546AU);
        #endif
        ...
        if(index < MIN_16B_RANGE && int_output[index] != NULL) {
            ...
            *int_memory[index - MIN_16B_RANGE] = ao_val;
            ...
            #if CFI_PROTECT
            CFI_EPILOGUE_TAG_PAC(0x19E6546AU);
            #endif
    ...
    virtual CommandStatus Select(const AnalogOutputInt32& command, uint16_t index) {
        #if CFI_PROTECT
        /* CFI: default, tag=0x064F1616U */
        CFI_PROLOGUE_TAG(0x064F1616U);
        #endif

        #if CFI_PROTECT
        CFI_EPILOGUE_TAG_PAC(0x064F1616U);
        #endif
        return CommandStatus::SUCCESS;
    }
    virtual CommandStatus Operate(const AnalogOutputInt32& command, uint16_t index, OperateType opType) {
        #if CFI_PROTECT
        /* CFI: default, tag=0x4C0BB0BCU */
        CFI_PROLOGUE_TAG(0x4C0BB0BCU);
        #endif
        ...
        if(index < MIN_32B_RANGE || index >= MAX_32B_RANGE)
            #if CFI_PROTECT
            CFI_EPILOGUE_TAG_PAC(0x4C0BB0BCU);
            #endif
            return CommandStatus::OUT_OF_RANGE;
    ...
    virtual CommandStatus Select(const AnalogOutputFloat32& command, uint16_t index) {
        ...
        #if CFI_PROTECT
        /* CFI: default, tag=0xE402B96CU */
        CFI_PROLOGUE_TAG(0xE402B96CU);
        #endif

        #if CFI_PROTECT
        CFI_EPILOGUE_TAG_PAC(0xE402B96CU);
        #endif
        return CommandStatus::SUCCESS;
    }
    virtual CommandStatus Operate(const AnalogOutputFloat32& command, uint16_t index, OperateType opType) {
        #if CFI_PROTECT
        /* CFI: default, tag=0xFEB60617U */
        CFI_PROLOGUE_TAG(0xFEB60617U);
        #endif
        ...
        if(index < MIN_32B_RANGE || index >= MAX_32B_RANGE)
            #if CFI_PROTECT
            CFI_EPILOGUE_TAG_PAC(0xFEB60617U);
            #endif
            return CommandStatus::OUT_OF_RANGE;
    ...
    virtual CommandStatus Select(const AnalogOutputDouble64& command, uint16_t index) {
        #if CFI_PROTECT
        /* CFI: default, tag=0xCA63B100U */
        CFI_PROLOGUE_TAG(0xCA63B100U);
        #endif

        #if CFI_PROTECT
        CFI_EPILOGUE_TAG_PAC(0xCA63B100U);
        #endif
        return CommandStatus::SUCCESS;
    }
    virtual CommandStatus Operate(const AnalogOutputDouble64& command, uint16_t index, OperateType opType) {
        #if CFI_PROTECT
        /* CFI: default, tag=0x8609C9AFU */
        CFI_PROLOGUE_TAG(0x8609C9AFU);
        #endif
        ...
        if(index < MIN_64B_RANGE || index >= MAX_64B_RANGE)
            #if CFI_PROTECT
            CFI_EPILOGUE_TAG_PAC(0x8609C9AFU);
            #endif
            return CommandStatus::OUT_OF_RANGE;
...
void update_vals(std::shared_ptr<IOutstation> outstation){
    #if CFI_PROTECT
    /* CFI: default, tag=0x8ABB11A6U */
    CFI_PROLOGUE_TAG(0x8ABB11A6U);
    #endif
    ...
    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0x8ABB11A6U);
    #endif
}
...
OutstationStackConfig create_config() {
    #if CFI_PROTECT
    /* CFI: default, tag=0xBFD4992CU */
    CFI_PROLOGUE_TAG(0xBFD4992CU);
    #endif
        ...
        while (getline(cfgfile, line)) {
            ...
            try {
                ...
                if (token == "database_size") {
                    getline(iss, token, '=');
                    #if CFI_PROTECT
                    CFI_EPILOGUE_TAG_PAC(0xBFD4992CU);
                    #endif
...
OutstationStackConfig parseDNP3Config() {
    #if CFI_PROTECT
    /* CFI: default, tag=0xCFEB475EU */
    CFI_PROLOGUE_TAG(0xCFEB475EU);
    #endif
...
// -------------------------------------------------------------------
    ...
    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0xCFEB475EU);
    #endif
    return config;
} 
...
{
    #if CFI_PROTECT
    /* CFI: default, tag=0xDEF454AAU */
    CFI_PROLOGUE_TAG(0xDEF454AAU);
    #endif
    ...
    openplc_log(log_msg);
    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0xDEF454AAU);
    #endif
}
...
void dnp3StartServer(int port) 
{
    ...
    #if CFI_PROTECT
    /* CFI: default, tag=0x084D42F3U */
    CFI_PROLOGUE_TAG(0x084D42F3U);
    #endif
    ...
    #if CFI_PROTECT
    CFI_EPILOGUE_TAG_PAC(0x084D42F3U);
    #endif
}
